...
function Calculator() {
  ...
  const handleNumber = (event) => {
    ...
    if (event.target.name === "0" && currentState === "0") {
      ...
      setCurrentState(event.target.name);
      setPrevState(event.target.name);
    } else if (["+", "-", "*", "/"].some((op) => currentState.includes(op))) {
  ...
  const handleOperator = (event) => {
    ...
    if (lastChar === event.target.name) {
      ...
      operatorsNoMinus.includes(lastChar) &&
      event.target.name === "-"
    ) {
      ...
      operatorsNoMinus.includes(lastChar) &&
      operatorsNoMinus.includes(event.target.name)
    ) {
      ...
      lastChar === "-" &&
      operatorsNoMinus.includes(secondLastChar) &&
      operatorsNoMinus.includes(event.target.name)
    ) {
  ...
  const calculate = () => {
    const result = parseFloat(eval(prevState).toFixed(8));
    setCurrentState(String(result));
    setPrevState(prevState + "=" + result);
